...
//With the update to handle bulk card call, will need to refactor this inorder to get only the card selected instead of referencing the state card
  const saveCardAPICall = () => {
	...
	{displaySave && <Button type="submit" size="large" color="success" onClick={handleSave} disabled={isSubmitting}>+ Add to Deck</Button> }
	{displayRemove && <RemoveCard name={props.name} cardCount={props.count} deckId={props.deckId}/>}
			<HtmlTooltip
        title={
          <React.Fragment>
            <Typography color="inherit">{props.name}</Typography>
			<b>{'Colors: '}</b><em>{props.colors}</em>.{' '} 
			<br></br>	
			<b>{'Types: '}</b>{props.type}.{' '}
			<br></br>
			<b>{'Cost: '}</b><em>{props.manaCost}</em>.{' '} 
			<br></br>				
